...
export default function AnalyticsPage() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 pb-20">
      {/* Header */}
      <header className="px-8 py-10 flex items-center justify-between max-w-7xl mx-auto">
        <div>
          <h1 className="text-[40px] font-black tracking-tight leading-none">House Points & Engagement</h1>
          <p className="text-slate-500 font-medium mt-2 text-lg">Leaderboards and analytics for house competitions.</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-6 py-3 bg-slate-50 hover:bg-slate-100 rounded-full font-bold text-sm transition-colors border border-slate-200">
            <TrendingUp className="w-4 h-4" /> Direct Adjustment
          </button>
          <button 
            onClick={() => setShowAwardModal(true)}
            className="flex items-center gap-2 px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-full font-bold text-sm transition-colors shadow-lg shadow-slate-200"
          >
      ...
      {/* Analytics Cards */}
      <div className="grid grid-cols-4 gap-6 px-8 max-w-7xl mx-auto mb-12">
        <motion.div 
      ...
      <div className="grid grid-cols-2 gap-12 px-8 max-w-7xl mx-auto">
        {/* House Leaderboard Chart */}
        <section>
          <div className="mb-8">
            <h2 className="text-[20px] font-black tracking-tight">House Leaderboard</h2>
            <p className="text-slate-400 text-[12px] font-black uppercase tracking-widest mt-1">Current house point standings.</p>
          </div>
          <div className="space-y-6">
            {houses.map((house) => {
              ...
              return (
                <div key={house.id} className="flex items-center gap-6">
                  <div className="w-20 text-right text-[14px] font-bold text-slate-400">{house.name}</div>
                  <div className="flex-1 h-12 bg-slate-50 rounded-xl overflow-hidden relative">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${percentage}%` }}
                      className={cn("h-full rounded-xl transition-all", config?.color)}
                    />
                  </div>
                </div>
              );
        ...
        <section>
          <div className="mb-4">
            <h2 className="text-[20px] font-black tracking-tight">Student Leaderboard</h2>
            <p className="text-slate-400 text-[12px] font-black uppercase tracking-widest mt-1">Top performing students based on house points.</p>
          </div>
          <table className="w-full">
             <thead>
                <tr className="text-left border-b border-slate-50">
                  <th className="py-4 text-[12px] font-black text-slate-400 uppercase tracking-widest">Rank</th>
                  <th className="py-4 text-[12px] font-black text-slate-400 uppercase tracking-widest px-4">Student</th>
                  <th className="py-4 text-[12px] font-black text-slate-400 uppercase tracking-widest">House</th>
                  <th className="py-4 text-[12px] font-black text-slate-400 uppercase tracking-widest text-right">Points</th>
                </tr>
             </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-3">
                          <div className={cn("w-10 h-10 rounded-full flex items-center justify-center font-black text-xs", house?.color.replace('bg-', 'bg-opacity-20 text-'))}>
                      ...
                      <td className="py-4">
                        <span className={cn("px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest inline-block", house?.color.replace('bg-', 'bg-opacity-10 text-'))}>
                           {house?.name || '---'}
                        </span>
                      </td>
                      <td className="py-4 text-right font-black text-[16px] tabular-nums">{student.points || 0}</td>
                    </motion.tr>
                  );
                })}
             </tbody>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            >
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-2xl font-black tracking-tight">Award Student Points</h3>
                <button onClick={() => setShowAwardModal(false)} className="text-slate-400 hover:text-slate-900 transition-colors">
              ...
              {!selectedStudent ? (
                <div className="space-y-6">
                  <form onSubmit={handleSearch} className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 pl-12 pr-4 font-bold text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/5"
                      value={searchQuery}
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                     <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center font-black">{selectedStudent.name.charAt(0)}</div>
                        <span className="font-black">{selectedStudent.name}</span>
                     </div>
                     <button onClick={() => setSelectedStudent(null)} className="text-[10px] font-black text-slate-400 uppercase underline">Change</button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col gap-2">
                       ...
                       <input 
                         type="number" 
                         className="bg-slate-50 border border-slate-100 rounded-xl p-3 font-bold text-sm"
                         value={points}
                  ...
                  <button 
                    ...
                    onClick={handleAward}
                    className="w-full btn-slate py-4 rounded-2xl flex items-center justify-center gap-2"
                  >
                    {isSubmitting ? 'Syncing...' : <> <Check className="w-5 h-5" /> Award Points </>}
                  </button>
                </div>
              )}
            </motion.div>
